...
class WeatherRepository {
    ...
    func mapAPIDatatoWeatherData( apiData: APIWeatherData, city: String) -> WeatherData {
        ...
        let daily : [Daily] = (0..<apiData.daily.time.count).compactMap { i in
            ...
            guard let forecastDate = WeatherFormatter.inputDateFormatter.date(from: timeString)else { return nil }
    ...
    func fetchAllCities() throws -> [WeatherData] {
        let descriptor = FetchDescriptor<WeatherData>( sortBy: [SortDescriptor(\WeatherData.city)] )
    ...
    func update(existingCity: WeatherData, with apiData: APIWeatherData) {
        ...
        appplyAPIData(to: existingCity, from: apiData)
        do{
            try context.save()
            print("🔄 \(existingCity.city) data updated and saved.")
        }
        catch{print("❌ Update and save error: \(error.localizedDescription)")}
    }
    ...
    private func appplyAPIData(to weatherData: WeatherData, from apiData: APIWeatherData) {
        ...
        print("🔄 \(weatherData.city) data updated.")
